...
import wandb
import datetime
#sys.path.append('/home/ahmad/MTO')
...
from mto.utils.utils import combine_x,combine_y
...
from mto.myutil import tag_to_image, trim_image
...
def train(**kwargs):
    ...
    if config_file and config_file.endswith(".json"):
        ...
        model_args, data_args, training_args, adapter_args = parser.parse_json_file(
            allow_extra_keys=True,  # <-- Add this
    ...
    if type(model_name_or_path) == list:
        model_name_or_path = "/".join(model_name_or_path) 
    ...
    router_dict = None
    init_router = None
    if cross_pt and use_saved_router:
    ...
    if not load_source_prompts and cross_pt:
    ...
    if method == "pt" or cross_pt:
        ...
        for name, prompt_tokens in encoders_prompts.items():
            ...
            if not cross_pt and prompt_exists and skip_if_prompt_exists: 
            ...
            if kwargs.setdefault("init_from_words", False):
                encoder.init_embs_from_words(model.get_input_embeddings())
            if not cross_pt and load_prompts: 
                ignore_if_prompt_not_exists = kwargs.setdefault("ignore_if_prompt_not_exists", False)
                # if not cross_pt or encoder.is_source:
                is_loaded = encoder.load(prompts_dir, 
                        prefix=prompts_prefix,
                        ignore_if_prompt_not_exists=ignore_if_prompt_not_exists,
                        as_saved=True,
    ...
    if use_wandb and save_router_image:
    ...
    if training_args.do_test:
        ...
        ################ Draw image
        def save_image(folder, model, score_dict, spec, title="pic", p_labels=None, 
            ...
            # title = "-".join(list(score_dict.keys())[0])
            title = title.strip("-")
            ...
            assert fpath.endswith("png"), "invalid mage name " + fpath
            new_im.save(fpath)
            ...
            for norm_method, gcmm, gmin, gmax, gntp in gen_combs:
                ...
                for gen_masks in gen_masks_list:
                    ...
                    for rm, mask in gen_masks.items():
                        ...
                        if test_num == "-1":
                            test_num = "all"
                        eval_folder_name = rv + "-" + exp_folder_name \
                                + "_num-" + test_num + "_" + str(ii) + "-" + mylogs.now 
                                #+ "-" + rm \
                        ...
                        eval_folder_name = sanitize_dirname(eval_folder_name)
                        ...
                        if cross_pt:
                            ...
                            if cross_pt:
                                save_image(eval_folder, model, {"router":router_scores}, 
                                        title = "router",
                                        spec = "router" + norm_method + "-" + str(gmin) \
                                                + "-" + str(gmax) \
                                                + " | {:.2f}".format(mean_score))
                            if init_router is not None:
                                ...
                                save_image(eval_folder, model, 
                                        {"init_router":init_router_scores},
                                        title = "router_init", spec=rm)
                            ...
                            if cross_pt:
                                save_image(eval_folder, model,
                                           {"mask": mask_matrix}, title="mask", spec="mask-"+rm)
                            mylogs.bp("effect")
                            if "-" in rm  and mask is not None:
                                ...
                                for _task, _score in mask_scores.items():
                                    ...
                                    if col < len(selected_cols_idx[task_index]):  
                                        ...
                                        pred_scores[test_key][task_index, col_index]=pcount
